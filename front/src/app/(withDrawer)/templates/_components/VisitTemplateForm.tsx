...
export default function VisitTemplateForm({
  ...
  return (
    <div className={`w-full mx-auto max-w-2xl p-6`}>
      <Link href="/templates" className="btn btn-square btn-ghost btn-sm">
        <IoArrowBack size={19} />
      </Link>
      <form action={action}>
        <input name="id" type="hidden" value={template?.id} />
        <FormLabel label="Template name" error={fieldErrors.name}>
          <input
            type="text"
            className="input input-bordered flex mb-4"
            name="name"
        ...
        <ul className="menu rounded-field w-full px-0">
          {titles.map((item) => (
            <li key={item.id}>
              <SectionView
                inputName="titles"
