...
public sealed class VisitReportDocument : IDocument
{
    ...
    public record PatientData(string FirstName, string LastName, string Idnp, DateOnly Birthday, string StreetAddress, string Job);
    ...
    public record VisitData(List<List<string>> Titles, SortedDictionary<string, string> Fields, DateTime CreatedAt, MedicData Medic, PatientData Patient, TriageData? Triage = null);
    ...
    private void ComposeFooter(IContainer footer)
    {
        footer.Column(col =>
        {
            col.Item().AlignLeft().Column(col =>
            {
                col.Item().Text($"Date: {_visitData.CreatedAt}").FontSize(10);
                col.Item().Row(r =>
                {
                    r.AutoItem().Text($"Medic: {_visitData.Medic.FirstName} {_visitData.Medic.LastName} ({_visitData.Medic.Specialty})").FontSize(12);
    ...
    void ComposeHeader(IContainer header)
    {
        header.Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text($"Numele, prenumele bolnavului: {_visitData.Patient.FirstName} {_visitData.Patient.LastName}").FontSize(12);
                ...
                col.Item().Text($"Locul de muncă (funcţia), studii: {_visitData.Patient.Job}").FontSize(12);
            });
        });
    }
    ...
    void ComposeTemplate(IContainer container, List<List<string>> titles, SortedDictionary<string, string> fields)
    {
        container.Column(column =>
        {
            ...
            foreach (var (key, val) in fields)
            {
                ...
                column.Item().Column(col =>
                {
                    ...
                    col.Item().Element(e => idxs.Last() > 0 ? e.PaddingLeft(16) : e).Text(titles[idxs.First()][idxs.Last()]).SemiBold();
    ...
    void ComposeTriage(IContainer container)
    {
        ...
        container.Column(col =>
        {
            ...
            col.Item().PaddingTop(6).Table(table =>
            {
                ...
                AddRow("Blood Pressure (systolic/diastolic)", $"{t.SystolicPressure}/{t.DiastolicPressure} mmHg");
