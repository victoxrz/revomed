...
{
    public VisitSuggestionRepository(PostgresDbContext context)
        : base(context) { }
    ...
    )
    {
        ...
        return _context
            .Suggestions.Where(s => s.TitlePath == titlePath && s.TemplateId == templateId)
            .Select(s => new
            {
                Suggestion = s,
